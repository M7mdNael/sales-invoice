...
export default function CreateReturnScreen() {
  ...
  const [selectedInvoice, setSelectedInvoice] = useState<SalesInvoice | null>(
    params.invoiceId
      ? salesInvoices.find((inv) => inv.id === params.invoiceId) ?? null
      : null
  );
  ...
  const handleSave = () => {
    ...
    const items: Omit<ReturnItem, "id">[] = selectedInvoice.items
      .map((item) => {
        ...
        if (qty > maxReturn) {
          Alert.alert(
            "Invalid Quantity",
            `Return quantity for ${item.productName} cannot exceed ${maxReturn} (already returned: ${alreadyReturned}).`
          );
          return null;
        }
        return qty > 0
    ...
    if (items.length === 0) {
      Alert.alert("No Items", "Please enter quantities to return.");
      return;
    }
    setSaving(true);
    ...
    return (
      <View style={styles.container}>
        <View style={styles.pickerHeader}>
          <Text style={styles.pickerTitle}>Select Invoice to Return</Text>
        </View>
        {salesInvoices.length === 0 ? (
          <View style={styles.emptyPicker}>
            <Feather name="file-text" size={40} color={C.textMuted} />
            <Text style={styles.emptyPickerText}>No invoices available</Text>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Pressable
                style={({ pressed }) => [
                  styles.invPickerItem,
                  pressed && styles.invPickerItemPressed,
                ]}
                onPress={() => handleSelectInvoice(item)}
              >
                <View style={styles.invPickerLeft}>
                  <Text style={styles.invPickerNum}>{item.invoiceNumber}</Text>
                  <Text style={styles.invPickerCustomer}>{item.customerName}</Text>
                  <Text style={styles.invPickerDate}>{formatDate(item.date)}</Text>
                </View>
                <Text style={styles.invPickerTotal}>
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.refCard}>
          <View>
            <Text style={styles.refLabel}>Original Invoice</Text>
            <Text style={styles.refValue}>{selectedInvoice.invoiceNumber}</Text>
          </View>
          <View>
            <Text style={styles.refLabel}>Customer</Text>
            <Text style={styles.refValue}>{selectedInvoice.customerName}</Text>
          </View>
          <Pressable
        ...
        <Text style={styles.fieldLabel}>Return Quantities</Text>
                ...
                <View style={styles.itemRow}>
                  <View style={styles.itemInfo}>
                    ...
                    <Text style={styles.itemSold}>
                      Sold: {item.quantity} · Max return: {maxReturn}
                    </Text>
                  ...
                  <TextInput
                    style={[
                      styles.qtyInput,
                      maxReturn === 0 && styles.qtyInputDisabled,
                    ]}
                    placeholder="0"
                    ...
                    onChangeText={(text) => {
                      ...
                      if (n > maxReturn) {
                        Alert.alert(
                          "Exceeds limit",
                          `Max return for ${item.productName} is ${maxReturn}`
                        );
                        return;
                      }
      ...
      <View style={[styles.footer, { paddingBottom: bottomPad }]}>
        ...
        >
          ...
          <Text style={styles.saveBtnText}>Create Return</Text>
        </Pressable>
      </View>
    </View>
  );
